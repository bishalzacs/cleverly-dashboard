    ...
    return (
      ...
      ${isActive
        ...
        >
            ...
            {isActive && (
                <div className="absolute top-0 left-0 w-1 h-full bg-brand-accent shadow-[0_0_20px_rgba(0,240,255,0.8)] z-10" />
            )}

            <div className="flex justify-between items-start mb-3">
                <div className="pr-3">
                    <h3 className={`font-semibold text-base leading-tight tracking-tight mb-1 transition-colors ${isActive ? "text-white" : "text-white/90 group-hover:text-white"}`}>
                ...
                <span className={`text-[10px] font-bold px-2.5 py-1 rounded-md uppercase tracking-wider backdrop-blur-md border ${isActive ? "bg-brand-accent/10 text-brand-accent border-brand-accent/20" : "bg-white/5 text-text-secondary border-white/5"
                    }`}>
                    {lead.status || "New"}
                </span>
            </div>
            ...
            <div className="flex flex-col space-y-1 mb-4 text-[11px] text-text-secondary font-medium">
                <span className="truncate flex items-center gap-1.5">
                    <svg className="w-3 h-3 opacity-70" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" /></svg>

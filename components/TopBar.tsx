    ...
    return (
        <header className="h-14 bg-surface-base border-b border-border-subtle flex-items-center justify-between px-6 sticky top-0 z-50 animate-fade-in">
            ...
            <div className="flex items-center space-x-5">
                ...
                <button
                    onClick={toggleTheme}
                    className="p-2.5 rounded-xl bg-surface-panel border border-border-subtle text-text-secondary hover:text-brand-primary hover:border-brand-primary/30 transition-all shadow-sm"
                ...
                <div 
                    ...
                    className="flex items-center space-x-3 bg-surface-panel border border-border-subtle px-4 py-2 rounded-full cursor-pointer hover:bg-red-500/10 dark:hover:bg-red-500/20 hover:border-red-500/30 group transition-all"
                    title="Sign Out"
                >
                    <div className="w-7 h-7 rounded-full bg-brand-accent/20 flex items-center justify-center text-xs font-black text-brand-accent border border-brand-accent/30 group-hover:bg-red-500 group-hover:text-white group-hover:border-red-500 transition-all shadow-sm">
                        {userEmail ? userEmail.charAt(0).toUpperCase() : "?"}
                    </div>
                    <span className="text-sm font-bold text-text-primary pr-1 max-w-[140px] truncate group-hover:text-red-500 transition-colors">
                        {userEmail || "Loading..."}
                    </span>
